...
export class CategoryService
{
  ...
  {
    return this.http.get(environment.API_ENDPOINT + '/checklist_category/items', { headers: this.postHeaders }).pipe(
      map(response => response.json().items));
  }

  getSingleCategory(id: number): Observable<Category[]>
  {
    return this.http.get(environment.API_ENDPOINT + `/checklist_category/${id}`, { headers: this.postHeaders }).pipe(
