...
export class QuestionPreService {
  ...
  getPreQuestions(checklistType:number) {
    return this.http.get(environment.API_ENDPOINT + `/questions_pre/items/${checklistType}`, { headers: this.postHeaders }).pipe(
  ...
  getChecklistItemsOnPreQuestionID(preQuestionID:number) {
    return this.http.get(environment.API_ENDPOINT + `/checklist/item/question_pre/${preQuestionID}`, { headers: this.postHeaders }).pipe(
      map(response => response.json().items))
  }
