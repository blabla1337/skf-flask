...
from skf.api.sprints.business import get_sprint_results_audit
...
@ns.route('/results/audit/<int:id>')
@api.doc(params={'id': 'The sprint id'})
@api.response(404, 'Validation error', message)
class ProjectSprintResultAuditItem(Resource):
    ...
    def get(self, id):
        """
        Returns sprint audit items.
        * Privileges required: **read**
        """
        validate_privilege(self, 'read')
        user_id = select_userid_jwt(self)
        result = get_sprint_results_audit(id, user_id)
