...
from skf.api.code.parsers import pagination_arguments, authorization, id_arguments, code_lang_arguments
...
class CodeLangItem(Resource):
    ...
    def post(self):
        ...
        code_query = (code_items.query.filter(code_items.code_lang == 'php'))
        code_page = code_query.paginate(page, per_page, error_out=False)
        #return code_items.query.filter(code_items.code_lang == code_lang).one(), 200, security_headers()
        return code_page, 200, security_headers()
