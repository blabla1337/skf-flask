...
def load_initial_data():
    ...
    try:
        ...
        c = ChecklistKB('8.1.1', 'Verify the application protects sensitive data from being cached in server components such as load balancers and application caches.', 8, False, 'https://cwe.mitre.org/data/definitions/542', 2)
        c.question_id = 46
        c.kb_id = 19
        db.session.add(c)
        db.session.commit()

        c = ChecklistKB('8.1.2', 'Verify that all cached or temporary copies of sensitive data stored on the server are protected from unauthorized access or purged/invalidated after the authorized user accesses the sensitive data.', 8, False, 'https://cwe.mitre.org/data/definitions/542', 2)
        c.question_id = 19
        c.kb_id = 145
        db.session.add(c)
        db.session.commit()
        ...
        c = ChecklistKB('14.2.1', 'Verify that all components are up to date, preferably using a dependency checker during build or compile time.', 14, False, 'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Vulnerable_Dependency_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/1026', 1)
        c.question_id = 65
        c.kb_id = 999
        db.session.add(c)
        db.session.commit()
        
        c = ChecklistKB('14.2.2', 'Verify that all unneeded features, documentation, samples, configurations are removed, such as sample applications, platform documentation, and default or example users.', 14, False, 'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Vulnerable_Dependency_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/1002', 1)
        c.question_id = 66
        c.kb_id = 283
        db.session.add(c)
        db.session.commit()
        
        c = ChecklistKB('14.2.3', 'Verify that if application assets, such as JavaScript libraries, CSS stylesheets or web fonts, are hosted externally on a content delivery network (CDN) or external provider, Subresource Integrity (SRI) is used to validate the integrity of the asset.', 14, False, 'https://cheatsheetseries.owasp.org/cheatsheets/Docker_Security_Cheat_Sheet.html,https://cheatsheetseries.owasp.org/cheatsheets/Vulnerable_Dependency_Management_Cheat_Sheet.html,https://cwe.mitre.org/data/definitions/714', 1)
        c.question_id = 66
        c.kb_id = 223
        db.session.add(c)
        db.session.commit()
